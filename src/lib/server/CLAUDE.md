...
This directory contains Zzz's backend server - a **reference implementation** using Hono and Deno. A Rust backend (`crates/zzz_server`) is in development, validated by integration tests in `test/integration/` that run the same assertions against both backends. See [crates/CLAUDE.md](../../../crates/CLAUDE.md).
...
| `testing_server_core.ts`        | Runtime-agnostic test-binary core. Owns env loading, forced `enable_test_actions: true`, `stub_password_deps`, `_testing_reset` registration via fuz_app's `create_testing_reset_actions` factory with the zzz `reset_state` closure (workspaces + terminals + optional `ZZZ_TESTING_SCRATCH_DIR`), daemon-info lifecycle, and graceful shutdown. Adapters supply runtime + serve + WS-upgrade-construction + signal-registration. |
| `testing_server_deno.ts`        | Deno test-binary adapter — binds `Deno.serve` + `hono/deno`'s module-level `upgradeWebSocket`. Counterpart to the Node entry; together they isolate the JS-runtime axis (Deno vs Node V8) on identical TS canonical surfaces. **Never ships in a release.**                                                                                                                                                                        |
| `testing_server_node.ts`        | Node/bun-runnable test-binary adapter — binds `@hono/node-server`'s `serve()` + `@hono/node-ws`'s two-phase `createNodeWebSocket(app)` / `injectWebSocket(server)`. Mirrors the Rust `testing_zzz_server` posture for cross-process integration tests against the TS canonical backend. **Never ships in a release** — reaches into fuz_app's `testing/cross_backend/` modules which throw on production load.                     |
